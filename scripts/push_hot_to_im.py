...
从 hot_items_raw 读取今日数据，按 hot_push_config 过滤后推送到飞书/钉钉/Telegram。
Webhook/Token 从 .env 读取。
"""
...
def main() -> int:
    ...
    for cfg in configs:
        if (cfg.get("im_channel") or "").lower() == "openclaw" or (
            (cfg.get("webhook_url") or "").strip().startswith("openclaw://")
        ):
            log(f"  [{cfg.get('category_name')}] openclaw 模式，跳过")
            continue
